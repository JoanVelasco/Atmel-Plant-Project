...
//static void rxCallbackAppl(uint8_t length);
...
static uint8_t usartRxBuffer[15];
static uint8_t ApplRxBuffer[15];
...
void APS_DataInd(APS_DataInd_t *indData)
{
	...
	{
		appWriteDataToUsart((uint8_t*)"success1\r\n",sizeof("success1\r\n")-1);
		if(indData->asdu==1)
		{
			PORTE |= (1<<PE3);
		}
		else if(indData->asdu==2)
		{
			PORTE &= ~(1<<PE3);
		}
		else
...
else if (CS_DEVICE_TYPE == DEV_TYPE_COORDINATOR)
{
